...
import { FlightOfferList } from '@app/components/FlightOfferList'
import { searchFlightsAdvanced } from '@app/lib/api/flights/flight-search'
import { FlightOffersAdvancedResponse, FlightOffersAdvancedSearchRequest } from '@fcm/shared/amadeus/clients/flight-offer-advanced'
...
export default function FlightSearchAdvancedPage() {
  ...
  const {
    ...
    mutationFn: async (searchParams: FlightOffersAdvancedSearchRequest) => {
      ...
      const cachedData = queryClient.getQueryData<FlightOffersAdvancedResponse>(['advancedFlightSearch', cacheKey])
  ...
  return (
    <Container maxWidth="md" sx={{ py: 4 }}>
      <Stack spacing={3}>
        ...
        <FlightSearchAdvancedForm onSubmit={handleSubmit} isLoading={isPending} />
        ...
        {searchResponse?.data && (
          <>
            {searchResponse.meta.oneWayCombinations && <Alert severity="info">Found {searchResponse.meta.oneWayCombinations.length} one-way combinations</Alert>}
            <FlightOfferList offers={searchResponse.data} dictionaries={searchResponse.dictionaries} />
          </>
        )}
      </Stack>
    </Container>
