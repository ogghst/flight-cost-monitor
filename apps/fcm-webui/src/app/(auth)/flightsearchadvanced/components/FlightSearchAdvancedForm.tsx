...
import {
  FLIGHT_OFFERS_DEFAULT_ADVANCED_VALUES,
...
type FlightSearchAdvancedFormProps = {
  onSubmit: (data: FlightOfferAdvancedSearchRequest) => void
  isLoading?: boolean
}

export function FlightSearchAdvancedForm({
  onSubmit,
  isLoading = false,
}: FlightSearchAdvancedFormProps) {
  const { control, handleSubmit } = useForm<FlightOfferAdvancedSearchRequest>({
